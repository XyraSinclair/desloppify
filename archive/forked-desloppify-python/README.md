...
Archived legacy Python fork.

This subtree is kept for historical reference while the active implementation
now lives in the Rust workspace at the repository root. Do not treat this
archive as the canonical CLI, packaging surface, or recommended install path.
...
## Historical Python Usage

Historically, developers used an editable Python install like this:

```bash
python3.11 -m venv .venv   # or any Python 3.11+
. .venv/bin/activate
python -m pip install -e "./archive/forked-desloppify-python[full]"
python -m desloppify --version
python -m desloppify scan --path .
python -m desloppify next
```

This section is preserved only to document how the archived Python fork used to
be run. The active `desloppify` CLI now lives at the repository root as the
Rust implementation, and `./scripts/desloppify-local` now targets that Rust CLI.

## Historical Agent Prompt

Do not copy the old Python prompt from this archive into an agent. The active
instructions for `desloppify` now live in the repository root and target the
Rust CLI.
