...
def _assessment_provenance_status(
    ...
    *,
    ...
    if runner not in blind_provenance_runner_choices():
        return AssessmentProvenanceModel(
            trusted=False,
...
def _print_import_error_hints(
    ...
    *,
    ...
    if (
        ...
        or "--attested-external currently supports runner" in joined
    ):
        print(
            colorize_fn(
...
def _apply_assessment_import_policy(
    ...
    *,
    ...
    if attested_external:
        ...
        allowed_attested = attested_external_runner_choices()
        if runner not in allowed_attested:
            return None, [
                "--attested-external currently supports runner provenance for: "
                f"{supported_attested_external_runners_text()}"
            ]
        policy = replace(
            policy,
