...
def _do_run_batches(args, state, lang, state_file, config: dict | None = None) -> None:
    ...
    try:
        runner_name = validate_batch_runner(str(getattr(args, "runner", "codex")))
    except UnsupportedBackendError as exc:
        print_error(str(exc))
        sys.exit(2)

    def _run_codex_backend(*, prompt, repo_root, output_file, log_file):
    ...
    if runner_name != "codex":
        print_error(
            f"unsupported batch runner backend '{runner_name}' (implemented: codex)"
        )
        sys.exit(2)

    runner_backend = BatchRunnerBackend(
        name=runner_name,
        run_batch=_run_codex_backend,
    )
    ...
    return review_batches_mod.do_run_batches(
        ...
        run_codex_batch_fn=_run_codex_backend,
