...
from desloppify.app.commands.review.exceptions import (
...
def _validate_runner(runner: str) -> str:
    """Validate review batch runner.

    Raises ``UnsupportedBackendError`` on unknown runner names.
    """
    return validate_batch_runner(runner)
...
def _require_batches(
    ...
    *,
    ...
    """Return investigation batches or raise ``BatchPrerequisiteError``."""
    batches = packet.get("investigation_batches", [])
    if isinstance(batches, list) and batches:
        return batches
    parts = ["packet has no investigation_batches."]
...
def do_run_batches(
    ...
    *,
    ...
    runner_backend: BatchRunnerBackend | None = None,
) -> None:
    """Run holistic investigation batches with a local subagent runner."""
    config = config or {}
    try:
        runner = _validate_runner(getattr(args, "runner", "codex"))
    except ReviewCommandError as exc:
        print(colorize_fn(f"  Error: {exc}", "red"), file=sys.stderr)
        sys.exit(2)
    if runner_backend is None:
        runner_backend = BatchRunnerBackend(
            name=runner,
            run_batch=lambda prompt, repo_root, output_file, log_file: run_codex_batch_fn(
                prompt=prompt,
                repo_root=repo_root,
    ...
    elif runner_backend.name != runner:
        print(
            colorize_fn(
                "  Error: runner backend mismatch "
                f"(args.runner={runner}, backend={runner_backend.name})",
                "red",
            ),
            file=sys.stderr,
        )
    ...
    max_parallel_batches = coerce_positive_int(
        getattr(args, "max_parallel_batches", None),
        default=3,
        minimum=1,
    )
    heartbeat_seconds = coerce_positive_float(
    ...
    except BatchPrerequisiteError as exc:
        print(colorize_fn(f"  Error: {exc}", "red"), file=sys.stderr)
        sys.exit(1)
    ...
    def _run_batch(*, prompt: str, output_file: Path, log_file: Path) -> int:
        return runner_backend.run_batch(
            prompt=prompt,
            repo_root=project_root,
            output_file=output_file,
            log_file=log_file,
