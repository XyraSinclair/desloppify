...
from desloppify.app.commands.review.exceptions import BatchPrerequisiteError
...
def test_import_attested_external_rejects_non_claude_runner(tmp_path, capsys):
    ...
    err = _render_import_load_error(exc.value, import_file=findings_path, capsys=capsys)
    assert "--attested-external currently supports runner provenance for:" in err
...
def test_require_batches_guides_rebuild_when_packet_has_no_batches():
    with pytest.raises(BatchPrerequisiteError) as exc_info:
        _require_batches(
            {"investigation_batches": []},
            suggested_prepare_cmd="desloppify review --prepare --path src",
        )
    msg = str(exc_info.value)
    assert "no investigation_batches" in msg
    assert "Regenerate review context first" in msg
    assert "review --run-batches --runner codex --parallel --scan-after-import" in msg
